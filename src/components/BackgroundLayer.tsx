...
export function BackgroundLayer({ config }: BackgroundLayerProps) {
  ...
  const convertedUrl = useMemo(
    () => (rawPath ? convertFileSrc(rawPath) : ""),
    [rawPath],
  );

  useEffect(() => {
    if (rawPath) {
      console.debug("[BackgroundLayer]", { rawPath, convertedUrl });
    }
  }, [rawPath, convertedUrl]);
  ...
  const onImageError = () => {
    console.warn("[BackgroundLayer] image failed", { rawPath, convertedUrl });
  };
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
      ...
      <div
        className="absolute inset-0 bg-cloud"
        style={{ opacity: dim }}
      />
    </div>
  );
